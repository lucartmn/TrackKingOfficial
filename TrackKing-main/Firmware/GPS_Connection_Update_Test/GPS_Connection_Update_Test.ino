...
void loop() {
  warmup();
  delay(1000);
  String postData = get_gps();
  update_serverdata(postData);
  
}

void warmup() {
  sendATCommand(800,"AT+CGNSPWR=1");
  delay(500);
  sendATCommand(800,"AT+CGATT=1");
  sendATCommand(800,"AT+SAPBR=0,1");sendATCommand(800,"AT+SAPBR=3,1,\"APN\",\"pinternet.interkom.de\"");
  sendATCommand(800,"AT+SAPBR=3,1,\"CONTYPE\",\"GPRS\"");
  sendATCommand(800,"AT+SAPBR=1,1");
  delay(2000); //Nur bei erstem Boot
}
...
String get_gps() {
  ...
  //Antwort aufnehmen
  String response = waitForResponse(500);
  //delay(2000);
  ...
  if (response.indexOf("+CGNSINF:") != -1) {
    ...
    Serial.println("Send Data:" + postData);
    delay(500);
    return postData;
  } else {
    delay(500);
...
void update_serverdata(String postData) {
  ...
  // Initialisiere HTTP und setze die URL und den Inhaltstyp
  sendATCommand(1200,"AT+HTTPINIT");
  sendATCommand(1500,"AT+HTTPPARA=\"CID\",1");
  sendATCommand(1500,"AT+HTTPPARA=\"URL\",\"http://lets-stream.eu/get_gps_data.php\"");
  delay(1000);
  sendATCommand(1500,"AT+HTTPPARA=\"CONTENT\",\"application/x-www-form-urlencoded\"");
  // Sende POST-Daten und führe die Aktion aus
  sendATCommand(800,("AT+HTTPDATA=" + String(postData.length()) + ",5000").c_str());
  sendATCommand(800,postDataChar);  // Verwende den umgewandelten const char* statt des String-Objekts
  sendATCommand(1000,"AT+HTTPACTION=1");
  // Lese die Serverantwort
  sendATCommand(2000,"AT+HTTPREAD");
  delay(1000);
  // Beende die HTTP-Sitzung
  sendATCommand(800,"AT+HTTPTERM");
}
